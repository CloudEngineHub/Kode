...
function getDirsFromGitRootToCwd(gitRoot: string, cwd: string): string[] {
  ...
  if (!rel || rel === '.') return [absoluteGitRoot]
...
export function readAndConcatProjectInstructionFiles(
  ...
  for (const file of files) {
    ...
    const finalBlock =
      suffixBytes >= remainingAfterSeparator
        ? truncateUtf8ToBytes(suffix, remainingAfterSeparator)
        : `${truncateUtf8ToBytes(block, remainingAfterSeparator - suffixBytes)}${suffix}`
