...
describe('UserToolResultMessage orphaned fallback', () => {
  ...
  test('caps matched tool React text result renderers outside verbose mode', async () => {
    const longResult = Array.from(
      { length: 85 },
      (_, index) => `node line ${index + 1}`,
    ).join('\n')
    const param = makeToolResultParam(
      'tool-use-long-react-node',
      'assistant content',
    )
    const message = makeToolResultMessage(param, { value: longResult })
    const toolUse = makeToolUseMessage('tool-use-long-react-node', 'FakeTool')
    const tool = makeTool({
      renderToolResultMessage(output) {
        return <Text>{(output as any).value}</Text>
      },
    })
    ...
    expect(terse).toContain('node line 80')
    expect(terse).toContain('... [truncated 5 lines] ...')
    expect(terse).not.toContain('node line 81')
    expect(verbose).toContain('node line 85')
  })
